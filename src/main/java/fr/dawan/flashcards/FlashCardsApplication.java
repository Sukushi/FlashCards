...
/*1*/			new Card("temoin", Category.TEST, "Question Témoin", "Réponse Témoin"),
...
/*10*/			new Card("heritage", Category.DEFINITION, "Qu'est ce qu'un \"heritage\" ?", "L'héritage établit une relation de généralisation-spécialisation qui permet d'hériter dans la déclaration d’une nouvelle classe (appelée classe dérivée, classe fille, classe enfant ou sous-classe), des caractéristiques (propriétés et méthodes), de la déclaration d'une autre classe (appelée classe de base, classe mère, classe parent ou super-classe)."),
			...
			new Card("dette_technique", Category.DEFINITION, "Qu'est ce que la \"dette technique\" ?", "La dette technique est un concept du développement logiciel inventé par Ward Cunningham en 1921. Le terme vient d'une métaphore, inspirée du concept existant de dette dans le domaine des finances et des entreprises, appliquée au domaine du développement logiciel. Elle désigne la difficulté à faire évoluer et à corriger un code source qui a été mal conçu initialement."),
			...
			new Card("controller", Category.DEFINITION, "Qu'est ce qu'un \"contrôleur\" ?", "Un contrôleur (Controller), contient la logique concernant les actions effectuées par l'utilisateur."),
			...
			new Card("genericite", Category.DEFINITION, "Qu'est ce que la \"généricité\" ?", "En programmation, la généricité (ou programmation générique), consiste à définir des algorithmes identiques opérant sur des données de types différents. On définit de cette façon des procédures ou des types entiers génériques."),
/*20*/			new Card("package", Category.DEFINITION, "Qu'est ce qu'un \"package\" ?", "En programmation, le terme paquetage(package), est le nom donné aux bibliothèques logicielles et sert aussi à la partition des espaces de noms."),
			new Card("invoquer", Category.DEFINITION, "En informatique, qu'est ce que \"invoquer\" signifie ?", "le terme \"invoquer\" est souvent utilisé dans le contexte de la programmation pour faire référence à l'action d'appeler ou d'exécuter une fonction, une méthode ou un programme .L'invocation est une partie fondamentale de la programmation, car elle permet de réutiliser du code en le décomposant en fonctions, méthodes ou programmes modulaires, que l'on peut invoquer à différents endroits de son code."),
...
/*30*/			new Card("specifique", Category.DEFINITION, "Qu'est ce que le terme \"spécifique\" signifie ?", "En résumé, le terme \"spécifique\" est utilisé pour mettre en évidence ce qui est distinct, particulier ou propre à une personne, à un objet, à un groupe ou à une situation particulière dans un contexte donné."),
...
/*50*/			new Card("back-end", Category.ARCHITECTURE, "Qu'est ce que le terme \"back-end\" signifie ?", "Le terme \"back-end\" se réfère à la partie invisible d'une application ou d'un site web, gérant les opérations en coulisses, comme la gestion des données, la logique métier et les interactions avec la base de données."),
...
/*60*/			new Card("POO", Category.SIGLE, "Que veut dire le sigle : POO ?", "Programmation Orientée Objet"),
			...
			new Card("MVC", Category.SIGLE, "Que veut dire le sigle : MVC ?", "Modèle-vue-contrôleur"),
...
/*70*/			new Card("SQL", Category.EXTENSION, "A quoi correspond cette extension : SQL", "Structured Query Language"),
...
/*80*/			new Card("jenkins", Category.SOFTWARE, "Quelle est la fonction de ce logiciel : Jenkins", "Jenkins est un outil d'intégration continue qui automatise le processus d'intégration, de test et de déploiement de logiciels."),
...
/*90*/			new Card("Symfony", Category.FRAMEWORK, "Quels sont les particularités de ce framework : Symfony", "")));
		}
		...
		try {
			passageService.insertPassage(1,8);
			passageService.insertPassage(1,14);
			passageService.insertPassage(1,19);
			passageService.insertPassage(1,46);
			passageService.insertPassage(1,50);
			passageService.insertPassage(1,60);
			passageService.insertPassage(1,61);
			passageService.insertPassage(1,70);
			passageService.insertPassage(1,73);
